...
def prepareAND(funcion):        #Hace la evaluacion inicial de la instruccion
    funcstr = funcion
    ...
    if len(funcion) == 3:       #Que contenga 3 Registros se
        for item in funcion:
            ...
            if ifRegistro(item):        #Que los registros existan
                ...
                try:
                    ...
                    msgBox = messagebox.showinfo('Error',"Error, inténtelo de nuevo", icon = 'warning')
            else:
        ...
        msgBox = messagebox.showinfo('Error',"Cantidad de registros inexistente", icon = 'warning')
    print(funcion)


def AND_fun(funcion, destino, valor1, valor2):      # Recibe la funcion, Rd,Rf1,Rf2
    tipoA(funcion, "00001", valor2, valor1, destino, "000000", "0", "0000", "1001")     #Envia los valores para crear
